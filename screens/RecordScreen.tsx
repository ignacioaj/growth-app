...
export default function RecordScreen() {
  ...
  const fetchRegistros = useCallback(async () => {
    const { data, error } = await supabase.from<Registro>('registros').select('*').order('date', { ascending: false });
    if (error) console.log('Error al leer registros:', error.message);
    else setRegistros(data || []);
  }, []);

  const onRefresh = useCallback(async () => { setRefreshing(true); await fetchRegistros(); setRefreshing(false); }, [fetchRegistros]);
  ...
  return (
    <ScrollView
      ...
      refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.trustBlue} colors={[colors.trustBlue]} />}
    >
      <View style={styles.header}><Text style={styles.headerTitle}>Registros</Text></View>
...
const styles = StyleSheet.create({
  container: { paddingHorizontal: 18, paddingTop: 28, paddingBottom: 56, backgroundColor: 'transparent', flexGrow: 1 },
  ...
  shadowWrapper: { borderRadius: 18, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.035, shadowRadius: 12, elevation: 2, backgroundColor: 'transparent' },
